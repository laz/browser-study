...
def handle_connection(conx):
    ...
    if SESSIONS.get(token) and SESSIONS.get(token).get('expires'):
        expires = SESSIONS.get(token).get('expires')
        expires_date = datetime.strptime(expires, COOKIE_EXPIRES_FORMAT)
    ...
    if "cookie" not in headers:
        expires_date = datetime.now(timezone.utc) + timedelta(seconds=10)
        date = expires_date.strftime(COOKIE_EXPIRES_FORMAT)
        session['expires'] = date
        template = "Set-Cookie: token={}; SameSite=Lax; Expires={};\r\n"
        response += template.format(token, date)
...
def do_request(session, method, url, headers, body):
    ...
    if method == "GET" and url == "/":
        ...
        params = form_decode(body)
        print('prev add', session)
        add_entry(session, params)
        print('after add', session)
        return "200 OK", show_comments(session)
    else:
