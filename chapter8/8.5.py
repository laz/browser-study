...
class Tab:
    ...
    def load(self, method, url, payload=None):
        print(f"load {method} {url} {payload}")
        self.history.append(History(method, url, payload))
        self.url = url
    ...
    def go_back(self):
        if len(self.history) > 1:
            if self.history[-2].method == "POST":
                ret = tkinter.messagebox.askyesno('ARE YOU SURE?', "YOU ARE MAKING THE SAME POST REQUEST BY DOING THIS. ARE YOU SURE??")
                if not ret:
                    return False
                
            self.history.pop()
            back = self.history.pop()
            self.load(back.method, back.url, back.payload)
