...
RUNTIME_JS = open("9.2.runtime.js").read()
...
class JSContext:
    ...
    def children_get(self, handle, s):
        children =  self.handle_to_node[handle].children
        ret = [self.get_handle(child) for child in children]
        return ret
    ...
    def insertBefore(self, parent, new, reference):
        parent_node = self.handle_to_node[parent]
        new_node = self.handle_to_node[new]
        reference_node = self.handle_to_node[reference]

        # find index
        idx = parent_node.children.index(reference_node)
        # put reference_node before index
        parent_node.children.insert(idx, new_node)
...
class Chrome:
    ...
    def paint(self):
        ...
        for i, tab in enumerate(self.browser.tabs):
            ...
            cmds.append(DrawText(bounds.left + self.padding, bounds.top + self.padding, 
